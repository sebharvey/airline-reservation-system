...
public static class FarePricer
{
    ...
    /// </summary>
    public static decimal ComputeOccupancyRatio(CabinInventory cabin) =>
        cabin.TotalSeats > 0
            ? Math.Clamp((decimal)(cabin.SeatsSold + cabin.SeatsHeld) / cabin.TotalSeats, 0m, 1m)
            : 0m;
    ...
    /// </summary>
    public static decimal ComputeDynamicPrice(decimal? min, decimal? max, decimal occupancyRatio)
    {
        var minVal = min ?? 0m;
        if (max is null || max <= minVal) return minVal;
        return Math.Round(minVal + (max.Value - minVal) * occupancyRatio, 2, MidpointRounding.AwayFromZero);
    }

    /// <summary>
    /// Same interpolation as <see cref="ComputeDynamicPrice"/> but for integer points values.
    /// </summary>
    public static int? ComputeDynamicPoints(int? min, int? max, decimal occupancyRatio)
    {
