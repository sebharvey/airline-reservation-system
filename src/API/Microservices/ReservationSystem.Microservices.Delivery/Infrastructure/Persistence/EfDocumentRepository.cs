...
/// <summary>
/// Entity Framework Core implementation of <see cref="IDocumentRepository"/>.
/// </summary>
public sealed class EfDocumentRepository : IDocumentRepository
{
    ...
    public EfDocumentRepository(
        ...
        ILogger<EfDocumentRepository> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task CreateAsync(Document document, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug("Inserted Document {DocumentId} into [delivery].[Document]", document.DocumentId);
    }
}
