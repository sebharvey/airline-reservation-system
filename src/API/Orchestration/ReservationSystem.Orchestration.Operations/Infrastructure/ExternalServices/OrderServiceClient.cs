...
public sealed class OrderServiceClient
{
        ...
        CancellationToken ct)
    {
        using var response = await _httpClient.GetAsync(
            $"/api/v1/orders/{Uri.EscapeDataString(bookingReference)}", ct);
        if (response.StatusCode == HttpStatusCode.NotFound) return null;
        response.EnsureSuccessStatusCode();

        var result = await response.Content.ReadFromJsonAsync<OrderMsOrderResult>(JsonOptions, ct);
        if (result?.OrderData is not { } data) return null;

        return ProjectToAffectedOrderDto(result, data, flightNumber, departureDate);
    }

    private static AffectedOrderDto? ProjectToAffectedOrderDto(
        ...
        string departureDate)
    {
        try
        {
            var bookingType = data.TryGetProperty("bookingType", out var bt) ? bt.GetString() ?? "Revenue" : "Revenue";

            string? loyaltyNumber = null;
            if (data.TryGetProperty("dataLists", out var dl) &&
                dl.TryGetProperty("passengers", out var paxList) &&
                paxList.ValueKind == JsonValueKind.Array)
            {
                foreach (var p in paxList.EnumerateArray())
                {
                    if (p.TryGetProperty("loyaltyNumber", out var ln) && ln.ValueKind != JsonValueKind.Null)
                    { loyaltyNumber = ln.GetString(); break; }
                }
            }

            var totalPointsAmount = 0;
            if (data.TryGetProperty("pointsRedemption", out var pr) &&
            ...
            if (data.TryGetProperty("payments", out var payments) && payments.ValueKind == JsonValueKind.Array)
            {
                ...
                {
                    if (payment.TryGetProperty("paymentReference", out var payRef) && payRef.ValueKind != JsonValueKind.Null)
                    { originalPaymentId = payRef.GetString(); break; }
                }
            }

            AffectedOrderSegmentDto? segment = null;
            if (data.TryGetProperty("orderItems", out var items) && items.ValueKind == JsonValueKind.Array)
            {
                foreach (var item in items.EnumerateArray())
                {
                    if (item.TryGetProperty("productType", out var pt) && pt.GetString() != "FLIGHT") continue;
                    ...
                    Guid.TryParse(item.TryGetProperty("inventoryId", out var invEl) ? invEl.GetString() : null, out var inventoryId);
                    segment = new AffectedOrderSegmentDto
                    {
            ...
            var passengers = new List<AffectedOrderPassengerDto>();
            if (data.TryGetProperty("dataLists", out var dl2) &&
