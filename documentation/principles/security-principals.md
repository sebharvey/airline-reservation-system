...
- **Orchestration API → Microservice calls are authenticated using Azure Function Host Keys.** Host keys are generated automatically when each Azure Function microservice is first deployed. The orchestration APIs (Retail API, Loyalty API, Airport API, Finance API, Operations API, Operations API), acting as microservice consumers, pass the host key on every inbound request via the `x-functions-key` HTTP header. Microservices reject any request that does not carry a valid host key.
...
- **Approved exception — Terminal HTTP debug modal:** The Contact Centre Terminal app (`src/Terminal`) includes an HTTP debug interceptor (`http-debug.interceptor.ts`) that captures full request and response payloads, which may include passenger PII (names, contact details, booking references). This is an intentional, contact-centre-only diagnostic tool accessible only to authenticated staff with the appropriate RBAC role. JWT tokens in `Authorization` headers are masked to `Bearer ***` before being stored in the debug log; PII in request/response bodies is displayed in full to allow agents to diagnose data issues. This exception is reviewed as part of the annual penetration test and must not be replicated in any customer-facing application.
