...
> **Indexes:** `IX_Order_BookingReference` (unique, filtered) on `(BookingReference)` WHERE `BookingReference IS NOT NULL` — enforces uniqueness of booking references whilst permitting multiple rows with a `NULL` booking reference (unconfirmed orders in `OrderInit` or `Draft` status). `IX_Order_ETicketNumber` (JSON, OPTIMIZE_FOR_ARRAY_SEARCH) on `OrderData` — covers the full JSON document with array-search optimisation, enabling efficient lookup of an order by any passenger's e-ticket number via `JSON_CONTAINS` or `JSON_PATH_EXISTS`.
