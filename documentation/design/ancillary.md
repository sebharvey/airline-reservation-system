...
- `SeatOfferId` values are base64-encoded strings of `{flightNumber}-{departureDate}-{cabinCode}-{seatNumber}` (e.g. `base64("AX001-2026-04-12-Y-35A")`). Generated by the Retail API at seatmap merge time. `BagOfferId` values are deterministic identifiers generated from `inventoryId + cabinCode + bagSequence`. Neither requires a dedicated offer storage table.
