...
| `GET` | `/v1/admin/flights/{inventoryId}/seatmap` | Return seatmap with pricing and seat availability for a flight (staff). Query params `flightNumber` and `aircraftType` (required); optional `cabinCode` to filter to a single cabin. Merges physical layout, seat pricing, and hold status. Staff JWT required. |
