...
| `GET` | `/v1/admin/inventory/{inventoryId}/inventory-orders` | Return all revenue orders for a flight, including per-cabin seat counts and a full order ledger (booking reference, cabin, fare family, base fare, tax, ancillaries per passenger). Used by the Terminal Stock Keeper flight detail screen. |
| `GET` | `/v1/admin/inventory/{inventoryId}/sales-performance` | Return daily sales performance for a flight over a rolling look-back window. Query param `days` (integer 1–90, default 21). Each day entry includes `orderCount`, `passengerCount`, `revenue` (fare + tax), and `ancillaryRevenue`. Used by the Terminal Stock Keeper flight detail screen to render trend charts. |
| `GET` | `/v1/admin/inventory/{inventoryId}/holds` | Return all active holds (revenue and standby) for a specific flight inventory record. Each hold entry includes hold type, cabin, passenger count, status, and creation timestamp. |
...
| `POST` | `/v1/ndc/ServiceList` | Accept an `IATA_ServiceListRQ` XML document; optionally parse `Query/SelectionCriteria/OfferRef/OfferRefID` (GUID) to derive cabin and flight context from the stored offer, and `Query/CabinPreferences/CabinType/CabinTypeCode/Code` (NDC cabin code M/W/C/F) to filter results; return an `IATA_ServiceListRS` XML document containing `Response/ServiceList/ALaCarteOffer` (one `ALaCarteOfferItem` per active SSR code with `OfferItemID` prefixed `SLI-`, `ServiceCode`, and indicative `UnitPriceDetail`) and `Response/DataLists/ServiceDefinitionList`; always returns `200` |
| `POST` | `/v1/ndc/OrderCreate` | Accept an `IATA_OrderCreateRQ` XML document; parse `Query/OrderItems/OfferItem/OfferRefID` (stored offer GUID), `Query/DataLists/PaxList` (named passengers with given name, surname, DOB, gender, contact details), and `Query/Payments/Payment/Method/PaymentCard` (optional card payment); orchestrate the full booking flow — reprice offer, create basket, add offer, update passengers, confirm with payment; return an `IATA_OrderCreateRS` XML document (HTTP `201`) containing `Response/Order` (`StatusCode=ISSUED`, `BookingRef`, `TotalAmount`, `OrderItem` elements with `StatusCode=PAYMENT_DONE`, `TicketDocInfo` per passenger) and `DataLists` (`FlightSegmentList`, `OriginDestinationList`, `PaxList`); returns `404` when the offer is not found, `410 Gone` when expired |
| `POST` | `/v1/ndc/OrderRetrieve` | Accept an `IATA_OrderRetrieveRQ` XML document; parse `Query/Filters/OrderFilter/BookingRef/ID` (booking reference) and `Query/Filters/OrderFilter/Pax/Individual/Surname` (lead passenger surname for guest authentication); retrieve the confirmed order via the Order microservice surname-validated retrieve endpoint; return an `IATA_OrderRetrieveRS` XML document containing `Response/Order` (`OrderID`, `BookingRef`, `StatusCode`, `TotalAmount`, `OrderItem` elements, `TicketDocInfo` per passenger) and `DataLists` (`FlightSegmentList`, `OriginDestinationList`, `PaxList`); returns `404` when the booking reference is unknown or the surname does not match |
| `POST` | `/v1/ndc/SeatAvailability` | Accept an `IATA_SeatAvailabilityRQ` XML document; parse `Query/OriginDestCriteria/OfferRefID` (stored offer GUID) and optional `Travelers`; resolve the offer to obtain `InventoryId` and `AircraftType`; concurrently fetch the physical seatmap layout (cabin, column, row, seat) and per-seat offer pricing from the Seat microservice; return an `IATA_SeatAvailabilityRS` XML document containing `Response/SeatAvailability/Flights/Flight/CabinList` (one `Cabin` per cabin with `ColumnList` and `RowList`/`Row`/`Seat` elements with `OccupationStatusCode` F=free, O=occupied and optional `OfferRef` with unit pricing) and `DataLists` (`FlightSegmentList`, `OriginDestinationList`); returns `404` when the offer or seatmap is not found |
...
| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET`  | `/v1/admin/schedule-groups` | Retrieve all schedule groups; returns `count` and per-group summary including `scheduleGroupId`, `name`, `seasonStart`, `seasonEnd`, `isActive`, `scheduleCount`; staff JWT required |
| `POST` | `/v1/admin/schedule-groups` | Create a new schedule group with `name`, `seasonStart`, `seasonEnd`, `isActive`, `createdBy`; returns the created group; staff JWT required |
| `PUT`  | `/v1/admin/schedule-groups/{scheduleGroupId}` | Update a schedule group's `name`, `seasonStart`, `seasonEnd`, `isActive`; staff JWT required |
| `DELETE` | `/v1/admin/schedule-groups/{scheduleGroupId}` | Delete a schedule group and all its flight schedules; staff JWT required |
| `GET`  | `/v1/admin/schedules` | Retrieve stored flight schedules; optional `?scheduleGroupId=` query parameter to filter by group; returns `count` and per-schedule summary including `scheduleId`, `scheduleGroupId`, route, times, `daysOfWeek`, aircraft type, validity window, `flightsCreated`, and `operatingDateCount`; staff JWT required |
| `POST` | `/v1/admin/schedules/ssim` | Import schedules from an IATA SSIM Chapter 7 plain-text file (`text/plain` body, required `?scheduleGroupId=` and optional `?createdBy=` query parameters); the Operations API parses all Type 3 scheduled-passenger leg records, converts them to the season schedule JSON payload format, and forwards to the Schedule MS `POST /v1/schedules`; returns `imported`, `deleted`, and per-schedule summary; staff JWT required |
| `POST` | `/v1/admin/schedules/import-inventory` | Generate `FlightInventory` and `Fare` records in the Offer MS from schedules stored in the Schedule MS; optional `?scheduleGroupId=` to limit to a specific group; request body supplies cabin seat counts for every aircraft type in the fleet as `aircraftConfigs[]{aircraftTypeCode, cabins[]{cabinCode, totalSeats}}`; the handler matches each schedule row to the correct aircraft config by `AircraftType`, enumerates operating dates, batch-creates inventory, then attaches fares from stored fare rules in the Offer MS; skips operating dates where inventory already exists; returns `schedulesProcessed`, `inventoriesCreated`, `inventoriesSkipped`, and `faresCreated`; staff JWT required |
...
| `POST` | `/v1/admin/flights/{inventoryId}/auto-assign-seats` | Auto-assign seats to all unassigned confirmed (non-standby) passengers on a flight; accepts `flightNumber`, `departureDate` (yyyy-MM-dd), and `aircraftType` in the request body; fetches the full seatmap from the Ancillary MS and the flight manifest from the Delivery MS; builds available seats per cabin, groups unassigned passengers by booking reference, and assigns whole booking groups to the same row where possible (random row distribution) falling back to individual assignment; updates the Delivery MS manifest for each seat assignment and syncs to the Order MS best-effort; returns `assigned` count, `failed` count, and per-passenger `outcomes[]` with `bookingReference`, `eTicketNumber`, `seatNumber`, `status` (`Assigned` or `Failed`), and `failureReason`; `404` if seatmap not found for the aircraft type; staff JWT required |
...
| `GET` | `/v1/admin/disruption/orders?flightNumber=&departureDate=` | Return affected orders for a (cancelled) flight sorted by IROPS rebooking priority (cabin F→J→W→Y, loyalty tier Platinum→Gold→Silver→Blue, booking date earliest first); used by the Terminal app IROPS rebooking screen to populate the per-booking list; staff JWT required |
| `POST` | `/v1/admin/disruption/rebook-order` | Rebook a single order affected by a flight cancellation; accepts `bookingReference`, `flightNumber`, `departureDate`; searches for the best available replacement flight (same logic as bulk cancel — direct first, connecting via LHR if needed, 7-day window, same or upgraded cabin); holds inventory, rebooks the order, reissues e-tickets, updates the manifest; returns `outcome` (`Rebooked` or `Failed`) with replacement details or failure reason; staff JWT required |
...
| `POST` | `/v1/inventory/hold` | Hold seats against a new or replacement booking (increments SeatsHeld; decrements SeatsAvailable); accepts optional `holdType` string (`Revenue` default, `Standby` for staff travel) and `standbyPriority` short integer — for standby holds inventory counters are not decremented (the passenger queues without consuming a seat), and a hold record is written with the supplied priority value |
