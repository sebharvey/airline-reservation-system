...
The Timatic Simulator mimics the IATA AutoCheck REST API for happy-path testing. It is called by the `DocumentVerification` service during online check-in (OCI) to verify passport, visa, and health document requirements, to validate Advance Passenger Information (APIS), and to run realtime gate checks via MRZ scan.

> **Important:** This is a simulator. No actual document, visa, or APIS validation is performed. All endpoints always return happy-path responses. It is not suitable for production use.
...
Validates passport, visa, and health document requirements for a passenger journey. Called at booking confirmation or OCI entry. Always returns `status: OK` with no visa and no health document required. Appends an ESTA advisory for journeys arriving at US airports.
...
Validates Advance Passenger Information when a passenger submits check-in data. Called during OCI submission. Always returns `apisStatus: ACCEPTED` with `fineRisk: LOW` and a generated audit reference.
...
Runs a realtime gate check when an agent scans the passenger's MRZ at the boarding gate. Parses ICAO 9303 TD3 Machine Readable Passport (MRP) lines and always returns `decision: GO`.
